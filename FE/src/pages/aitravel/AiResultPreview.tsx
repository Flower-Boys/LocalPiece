...
import type { CategoryKey, TripResponse, Course } from "@/types/aiTravel";
import { Map as MapIcon } from "lucide-react";
import {getTourCommon} from "@/api/tour"

const AiResultPreview: React.FC = () => {
  const nav = useNavigate();
  const { state } = useLocation() as { state?: { selected?: CategoryKey[]; data?: TripResponse }};
  ...
  const courseToItem = (course: Course, idx: number) => {
   ...
   const cityMatch = addr.match(
     /(경주시|포항시|안동시|구미시|김천시|영주시|영천시|상주시|문경시|경산시|의성군|청송군|영양군|영덕군|청도군|고령군|성주군|칠곡군|예천군|봉화군|울진군|울릉군)/
   );
    ...
    <main className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 lg:px-8">
      ...
      return (
        <RouteCard
          key={item.id}
          item={withCover}
          to="/ai/travel/route-detail"
          state={{ course: trip!.courses[idx] }} // ✅ 코스 원본을 상세로 전달
        />
      );
    })}
        </div>
